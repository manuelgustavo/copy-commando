...
private:
	...
	//HBITMAP		m_h_mem_bitmap;
	//CBitmap*	m_pOldBitmap;				// original bitmap
	//CBitmap*	m_old_bitmap;				// original bitmap
	HBITMAP		m_h_old_bitmap;
...
public:
	...
	CMemDC(CDCHandle* pDC, const CRect* pRect = NULL) : CDCHandle()
	{
		...
		//{
			...
			pDC->LPtoDP(&m_rect);
			m_MemBitmap.CreateCompatibleBitmap(pDC->m_hDC, m_rect.Width(), m_rect.Height());
			//CDCHandle
			//m_pOldBitmap = SelectObject(&m_MemBitmap);
			m_h_old_bitmap = SelectBitmap(m_MemBitmap);
			//m_h_old_bitmap = SelectObject(m_h_mem_bitmap);
			SetMapMode(pDC->GetMapMode());
			SIZE aux;
			pDC->GetWindowExt(&aux);
			SetWindowExt(aux);
			pDC->GetViewportExt(&aux);
			SetViewportExt(aux);
			pDC->DPtoLP(&m_rect);
